...
public class AntennaArray {
    ...
    /**
     * Check whether an antenna design lies within the problem's feasible region. A
     * design is a vector of n_antennae anntena placements. A placement is a
     * distance from the left hand side of the antenna array. A valid placement is
     * one in which 1) all antennae are separated by at least MIN_SPACING 2) the
     * aperture size (the maximum element of the array) is exactly n_antennae/2.
     */
    ...
    public int getNumberOfAntennae() {
        return n_antennae;
    }

    /**
     * Evaluate an antenna design returning peak SSL. Designs which violate problem
     * constraints will be penalised with extremely high costs.
     * 
     * @param design A valid antenna array design.
     */
    public double evaluate(double[] design) {
        if (design.length != n_antennae)
            throw new RuntimeException("AntennaArray::evaluate called on design of the wrong size. Expected: "
                    + n_antennae + ". Actual: " + design.length);
        if (!is_valid(design))
